...
function isAlreadyResponList(groupID, key, _db) {
    ...
    Object.keys(_db).forEach((x) => {
        if (_db[x].id === groupID && _db[x].key.toLowerCase() === key.toLowerCase()) {
            found = true
        }
    })
    return found
...
function isAlreadyResponListAllGroup(_db){
...
*/
function  expiredCheck(alpha, _dir) {
    setInterval(async () => {
        // Store the current time for comparison
        const now = Date.now();
        // Create an array to hold expired items' indices
        const expiredItems = [];

        // Check for expired items
        Object.keys(_dir).forEach((i) => {
            if (now >= _dir[i].expired) {
                expiredItems.push(i); // Collect expired item indices
            }
        });

        // Handle expired items
        for (const position of expiredItems) {
            console.log(`Sewa expired: ${_dir[position].id}`);
            await alpha.sendMessage(_dir[position].id, { text: `Masa sewa di grup ini telah habis, bot otomatis keluar!` });
            await alpha.groupLeave(_dir[position].id);

            // Remove expired item from _dir
            _dir.splice(position, 1);
        }

        // Write updated _dir back to the file only once
